...
def set_numbers(num_or_sym:str, numbers_sym_array:list, operation_history_array:list, sym1:list, sym2:list, display_text, result_list:list) -> None:
    ...
    if num_or_sym in symbols_list:
        print("result handler debbuging", num_or_sym, numbers_sym_array)
        if result_list and (not operation_history_array) and (not numbers_sym_array) and (not (num_or_sym in ['(', ')'])) :
    ...
    if num_or_sym == "=":
        ...
        try: 
            ...
            display_operation(last_display_array, display_text, 'Error in operation (clr to continue)')
